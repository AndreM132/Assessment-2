...
def one(input):
    input1 = input
    numrep = 3
    ans = "".join([y*numrep for y in input1])
    return ans
...
def four(input1, input2):
    input1 = input("First: ").split()
    input2 = input("Second: ").split()
    new_s = ""
    for i in range(len(input1)):
        new_s += (input1[i] + input2[i])
    return new_s
...
import random
def five():
    randomrange = random.sample(range(100,201,2),5)
    return(randomrange)
...
def nine(string1, string2):
    if string1 == 'True' and string2 == 'True':
         return True
    elif string1 == 'False' and string2 == 'False':
         return False
    else:
        return False
